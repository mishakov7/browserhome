...
export default function Note(props: any) {
  ...
  const changeText = (noteValue: string) => {
    ...
    let noteWords = noteValue.split(" ");
    let updatedWords = "";

    noteWords.forEach((word) => {

      if (word.match(regex) && (word.includes("http") || word.includes("https"))) {
        let anchor = document.createElement("a");

        if (word.includes("https")) {
          anchor.textContent = word.split("https://")[1];
        
        } else if (word.includes("http")) {
          anchor.textContent = word.split("http://")[1];
    ...
    if (ctx.t_sticky > 0) {
      setCtx({ type: "tutorial_sticky", step: 2 });
    }
  }
      ...
      onStart={(e) => props.changeLayer(props.idx)}>
        <div onClick={(e) => props.changeLayer(props.idx)} ref={nodeRef} className={'sticky note ' + (props.isSelected ? "top-sticky" : null)}>
            ...
            <div className={props.storage.rotation + ' note-wrapper sticky-' + noteColor}>
                <div ref={textareaRef} suppressContentEditableWarning={true} contentEditable={true} onBlur={(e) => { if (e.target.textContent) { changeText(e.target.textContent) }}}className="textarea note-text">{notetext}</div>
