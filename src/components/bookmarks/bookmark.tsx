...
const Bookmark = (props: any) => {
  ...
  return (
    ...
    <li ref={bookmarkRef} style={{opacity}} data-handler-id={handlerId} className={'bookmark ' + (props.isMovingLeft ? ' dragging-left ' : '') + (props.isMovingRight ? ' dragging-right' : '')} data-link={props.link.split("//")[1]}>
      ...
      <button onClick={() => { props.parentElmt.current.classList.remove("bookmarks-list-creator"); toggleAlert(); }} className='remove-button'>
...

      {
        ...
        <Creator 
            ...
            handleCreator={() => props.handleDelete(props.bookmarkIdx)}
            handleMouseLeave={() => { showAlert ? setAlert(false) : setAlert(false) }}
